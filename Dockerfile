...
# Map Reports folder (kept as-is)
VOLUME ${TOMCAT_HOME}/webapps/birt

# -------------------------
# Fonts: FIX (the important part)
# -------------------------

# Keep mundial if you want, but DO NOT install arial.ttf into system fonts
ADD mundial.ttf /usr/share/fonts/truetype

# Stage fonts into a dedicated directory BIRT can scan
RUN set -euo pipefail; \
    ...
    # Just in case something still drops it in there
    rm -f "${BIRT_FONTS_DIR}/arial.ttf" || true; \
    fc-cache -f -v

# Generate a predictable fontsConfig.xml for BIRT in WEB-INF/classes
RUN set -euo pipefail; \
...
# -------------------------
# Your app/report assets (kept as-is)
# -------------------------
