...
ARG BIRT_RUNTIME_DATE=202506110821
...
# Diagnostics: locate BIRT fonts plugin and list configs (non-fatal)
RUN echo "[diag] Searching fonts plugin under ${TOMCAT_HOME}/webapps/birt/WEB-INF/platform/plugins" \
 && find ${TOMCAT_HOME}/webapps/birt/WEB-INF/platform/plugins -maxdepth 1 -type f -name 'org.eclipse.birt.report.engine.fonts_*.jar' -print 2>/dev/null | head -n 5 \
 && for J in $(find ${TOMCAT_HOME}/webapps/birt/WEB-INF/platform/plugins -maxdepth 1 -type f -name 'org.eclipse.birt.report.engine.fonts_*.jar' 2>/dev/null | head -n 2); do \
      echo "[diag] Listing font configs in $J"; \
      unzip -l "$J" | grep -E 'fontsConfig.*\\.xml|font.*\\.properties' || true; \
    done || true
